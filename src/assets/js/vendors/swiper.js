...
function A11y(_ref) {
    ...
    const init = () => {
        ...
        addElLabel($wrapperEl, 'Slider Label Title');      
        swiper.slides.each((slideEl, index) => {
            const $slideEl = $(slideEl);
            if($wrapperEl.attr('role') !== 'tablist'){
                if((swiper.wrapperEl.localName == "ul" || swiper.wrapperEl.localName == "ol") !== true){
                    addElRole( $wrapperEl, 'list');
                    addElRole( $slideEl, 'listitem');
                }
            }
        });
