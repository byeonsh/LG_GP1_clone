...
const selectRequiredCheck = () => {
  for (let index = 0; index < submitButton.length; index++) {
    submitButton[index].addEventListener('click', e => {
      ...
      e.stopPropagation();
      const countInputs = submitButton[index].closest('.CS0051__container').querySelectorAll('select');
      // console.log(e.target);
...
const inputRequiredCheck = () => {
  for (let index = 0; index < submitButton.length; index++) {
    submitButton[index].addEventListener('click', e => {
      e.stopPropagation();
      const countInputs = submitButton[index].closest('.CS0051__container').querySelectorAll('input[type="text"]');
