...
const resetList = el => {
  el.forEach(c => {
    const list = c.querySelector('.c-bloglList .c-list');
    if (list) {
      const isMobile = mobileSize();
...
const clickMore = event => {
  ...
  const list = btn.closest('.component').querySelector('.c-bloglList .c-list');
  const btnArea = btn.closest('.c-bloglList .more-btn');
...
// init
function init() {
  const component = document.querySelectorAll('.AL0001');
  if (beforeLaunch(component)) return false;
  component.forEach(el => {
    // init
    const list = el.querySelector('.c-bloglList .c-list');
    if (list) {
      list.dataset.first = true;
    }
  });

  const moreButtons = document.querySelectorAll('.AL0001 .c-bloglList .more-btn button');
