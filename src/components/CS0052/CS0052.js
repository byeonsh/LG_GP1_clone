...
function init2() {
  const component = document.querySelectorAll('.CS0052');
  if (beforeLaunch(component)) return false;
  const viewedProductCarousel = document.querySelectorAll('.CS0052 .type-card__slide .swiper');
...
const toastPopup = function () {
  const component = document.querySelector('.CS0052 .c-with-toast');
  const toastBox = component.querySelector('.toast');
  const toastOpen = document.querySelectorAll('.CS0052 .toastOpen');
  const toastCloseBtn = document.querySelector('.CS0052 .closeBtn');
