...
const resetList = el => {
  el.forEach(c => {
    const list = c.querySelector('.c-category-contents .c-list');
    if (list) {
      const isMobile = mobileSize();
...
const clickMore = event => {
  ...
  const list = btn.closest('.component').querySelector('.c-category-contents .c-list');
  const btnArea = btn.closest('.c-category-contents .more-btn');
...
// init
function init() {
  const component = document.querySelectorAll('.GN0005');
  if (beforeLaunch(component)) return false;
  component.forEach(el => {
    // init
    const list = el.querySelector('.c-category-contents .c-list');
    if (list) {
      list.dataset.first = true;
    }
  });

  const moreButtons = document.querySelectorAll('.GN0005 .c-category-contents .more-btn button');
